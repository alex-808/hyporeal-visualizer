...
window.onSpotifyWebPlaybackSDKReady = () => {
  function runVisuals() {
    ...
    function handleEvents() {
      ...
      if (newHits.segment) {
        ...
        sceneInfo1.planeBackrow = trackData.segments[segmentCounter]['pitches'];

        [sceneInfo3.timbreSum1, sceneInfo3.timbreSum2] = calcTimbreSums();
    ...
    function getSectionEffect(maxRandom) {
      ...
      switch (random) {
        case 0:
        case 1:
          if (effects.previous === 'glitch') return getSectionEffect(10);
    ...
    function renderSceneInfo(sceneInfo) {
      ...
      visuals.renderer.setViewport(left, positiveYUpBottom, width, height);

      if (sceneInfo === sceneInfo4) {
        if (effects.masking === false) {
          visuals.renderer2.setScissor(left, positiveYUpBottom, width, height);
          visuals.renderer2.setViewport(left, positiveYUpBottom, width, height);
  ...
  function updateDivisionCounters() {
    segmentCounter = findNextDivision(trackData.segmentsStart, segmentCounter);
    ...
    sectionCounter =
      findNextDivision(trackData.sectionNearestBarStart, sectionCounter) - 1;
  }
  ...
  function checkForHits() {
    ...
    if (
      elapsedTime + elapsedMilliseconds >=
      trackData.sectionNearestBarStart[sectionCounter + 1] - syncCompensation
    ) {
